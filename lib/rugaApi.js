...
const vidhot = async () => new Promise((resolve, reject) => {
...
const howgay = async () => new Promise((resolve, reject) => {
    axios.get(`${link}/api/howgay`)
    .then((res) => {
        const text = `Deskripsi: ${res.data.desc}\n\nPersen: ${res.data.persen}%`
        resolve(text)
    })
    .catch((err) =>{
        reject(err)
...
const howbucin = async () => new Promise((resolve, reject) => {
...
const katasulit = async (negara) => new Promise((resolve, reject) => {
    axios.get(`https://api.i-tech.id/tools/twist?key=YiKNUe-ElDPJV-kSbtMj-cGwqPF-DBoL5w&negara={negara}`)
    .then((res) => {
...
const infogempa = async () => new Promise((resolve, reject) => {
    axios.get(`${link}/api/infogempa`)
    .then((res) => {
        const text = `*Waktu:* ${res.data.waktu}\n\n*Lokasi:* ${res.data.lokasi}\n*Koordinat:* ${res.data.koordinat}\n*Magnitude:* ${res.data.magnitude} SR\n*Kedalaman:* ${res.data.kedalaman}\n*Daerah Terdampak:* ${res.data.potensi}\n\nA F S BOT`
        resolve(text)
    })
    .catch((err) =>{
        reject(err)
...
const bmkgpict = async (url) => new Promise((resolve, reject) => {
    axios.get(`https://arugaz.herokuapp.com/api/infogempa`)
    .then((res) => {
		if (res.data.error) resolve('https://ews.bmkg.go.id/tews/data/20201129214828.mmi.jpg')
...
const kbbi = async (query) => new Promise((resolve, reject) => {
    axios.get(`https://api.i-tech.id/tools/kbbi?key=YiKNUe-ElDPJV-kSbtMj-cGwqPF-DBoL5w&query={query}`)
    .then((res) => {
        const text = `Status: ${res.data.status}\n\nHasil: ${res.data.result}`
        resolve(text)
    })
    .catch((err) =>{
        reject(err)
...
const kataninja = async (kata) => new Promise((resolve, reject) => {
	axios.get(`https://api.i-tech.id/tools/ninja?key=YiKNUe-ElDPJV-kSbtMj-cGwqPF-DBoL5w&kata={kata}`)
	.then((res) => {
		resolve(res.data.result)
	})
	.catch((err) => {
